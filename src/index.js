    ...
    if (result.length > 10) {
        ...
        return result.map(({ name, flags }) => {
                ...
                return result.map(({ name, capital, population, flags, languages }) => {
                    ...
                    const infoCountry = `<li><img src='${flags.svg}' width=30 height=20> ${name.official}</li><li>capital: ${capital.join('')}</li><li>population: ${population}</li><li>languages: ${Object.values(languages).join(' ')}</li>`;
